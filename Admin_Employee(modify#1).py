...
def employeeWindow():
    ...
    def timein1(empid_value, current_time, current_day):
        try:
            ...
            if result:
                ...
                print("Access Failed! {}".format(Error))
        finally:
                if con.is_connected():
                    con.close()
    ...
    leaveForm = tkinter.Button(empwind, text = 'Leave Form').grid(row=6, column=1, padx=10, pady=0)
...
def adminui():
    ...
    adwin_btn2=tkinter.Button(adwin, text="Time out Records")
    adwin_btn2.grid(row = 1, column = 1,  padx=10, pady=10)

    adwin_btn3=tkinter.Button(adwin, text="Leave Records")
